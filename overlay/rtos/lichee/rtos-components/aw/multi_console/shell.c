...
void shell_thread_entry(void *param)
{
	...
	while (1) {
		...
		if(read_char == '\n' || read_char == '\r') {
			...
			do {
				memset(output_str, 0, sizeof(output_str));
				ret = cli_port_parse_string(input_str, output_str,
							    sizeof(output_str));
				if (output_str[0] != '\0')
					cli_console_write(console, output_str,
							  strlen(output_str));
			} while (ret);
			line_cur = line_pos = 0;
			if (console->dev_ops->prefix)
				cli_console_write(console, console->dev_ops->prefix,
						  strlen(console->dev_ops->prefix));
			memset(input_str, 0, sizeof(input_str));
		} else {
