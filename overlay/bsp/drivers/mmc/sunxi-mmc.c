...
static void sunxi_mmc_dump_errinfo(struct sunxi_mmc_host *host)
{
	...
	u32 err = host->int_sum & SDXC_INTERRUPT_ERROR_BIT;

	/*
	 * A host using MMC_CAP_NEEDS_POLL probes an empty slot periodically.
	 * Response timeouts from the discovery/status commands are the expected
	 * indication that no card is present, not a controller error.
	 */
	if ((host->mmc->caps & MMC_CAP_NEEDS_POLL) &&
	    err == SDXC_RESP_TIMEOUT) {
		switch (cmd->opcode) {
